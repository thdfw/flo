        ...
        for h in range(HORIZON_HOURS+1):
            for node in self.nodes[h]:
                ...
                if h<ADD_MIN_HOURS:
                    ...
                    while True:
                        if thermoc==1:
                            if toptemp-TEMP_DROP_F < MIN_TOP_TEMP_F:
                                break
                            toptemp = toptemp-TEMP_DROP_F
                            thermoc = NUM_LAYERS
                ...
                if h < ADD_MAX_HOURS:
                    ...
                    while True:
                        if thermoc==NUM_LAYERS:
                            if toptemp+TEMP_LIFT_F > MAX_TOP_TEMP_F:
                                break
                            toptemp = toptemp+TEMP_LIFT_F
                            thermoc = 1
                        node_next = Node(h+1, thermocline=thermoc, top_temp=toptemp)
                        energy_to_store = node_next.energy - node.energy
                        if  energy_to_store + self.load[h] > HP_POWER_KW:
                            thermoc += -1
                            if thermoc == 0:
